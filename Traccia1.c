...
int main () {
	...
	printf("Tutto ciò che devi fare è inserire un numero intero.\n");
	...
	area_quadrato = D * D;
	printf("\nL'area di un quadrato con un lato di cm %d è: %d\f", D, area_quadrato);
	
	//calcola l'area del cerchio
	float raggio = D / 2.0;
	area_cerchio = pigreco * (raggio * raggio);
	printf("\nL'area di un cerchio con un diametro di cm %d è: %.2f\f", D, area_cerchio);
	
	//calcola l'area del triangolo
	area_triangolo = ((sqrt(3) / 4)* D * D);
	printf("\nL'area di un triangolo equilatero con un lato di cm %d è: %f\f", D, area_triangolo);
